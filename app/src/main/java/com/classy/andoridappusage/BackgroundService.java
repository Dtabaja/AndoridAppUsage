...
public class BackgroundService extends JobIntentService {
    ...
    protected void onHandleWork(Intent intent) {
        ...
        HashMap<String, Integer> appUsageMap = Utils.getTimeSpent
                (this, null, beginTime, endTime);

        String currentRunningPackageName = null;
        List<String> list = appUsageMap.keySet().stream().filter
                (s -> s.startsWith("current")).collect(Collectors.toList());
        if(list.size() > 0) {currentRunningPackageName =
                list.get(0).replaceFirst("current", "");}

        for(int i = 0; i < trackedAppInfos.size(); i++) {
            TrackedInfo trackedAppInfo = trackedAppInfos.get(i);
            ...
            if(appUsageMap.containsKey(packageName)) {
                ...
                if((usageTime > allowedTime && isUsageExceeded == 0) ||
                    ...
                    try {
                        internalDataBase.setIsUsageExceeded(packageName);
